...
function dpm_init(data::RawData, priors::InputPriors, params::InputParams; xmats_out=false)
    ...
    @inbounds for i in 1:n
        ...
        dstar[i] = rand( truncated(Normal(0, 1), lower[i], upper[i]) )
    end
    ...
    if input.params.model == "blocked"
        ## ensure all components 1:J exist in njs
        for j in 1:priors.prior_dp.J
            get!(njs, j, 0)
        end
        ## initialize weights
        for k in sort(collect(keys(state_dp.njs)))
            get!(state_dp.ws, k, BlockedWeights())
        end
